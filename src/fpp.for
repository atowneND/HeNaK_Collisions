...
C            ELSEFL - FLAG TO KEEP TRACK OF NESTED IF/ELSE BLOCKS
...
C
      ...
      LOGICAL WRITE, comment
      COMMON /SOURCE/ SOURCE
      COMMON /WRITE/ NESTL, WRITE, comment, nestc
      integer in,out,argc
      character    argv*64
...
*     process directives entered via command line arguments
      argc = iargc()
      DO i=1,argc
         call getarg(i,argv)
         if (argv(1:1).eq.'-') then
...
6     READ(in,'(A)',END=7) LINE
      SOURCE=LINE
      IF (LINE(1:1).EQ.'$') THEN
         CALL COND
         write (out,*)   ! write a blank line
      ELSEIF (WRITE .and. .not.comment) THEN ! IF LAST CONDITIONAL WAS TRUE THEN WRITE LINE
         WRITE(out,'(A)') LINE(:IBLANK(LINE,80))
      else
         write(out,*)    ! write a blank line
      ENDIF
      GOTO 6
...
C     CHECK TO MAKE SURE ALL IF BLOCKS and comment blocks ARE CLOSED
      if (nestl.ne.0) then
         WRITE(*,'('' FATAL - $IF BLOCK NOT CLOSED.'')')
         call exit(3)
      else if (comment) then
         WRITE(*,'('' FATAL - $COMMENT BLOCK NOT CLOSED.'')')
         call exit(3)
      end if
...
*     check first for comment
      if (line(1:7).eq.'COMMENT') then
         if (.not.comment) then
           comment = .true.
           nestc = nestl
           return
            ...
            WRITE(*,'('' '',A//
     -       '' FATAL - UNMATCHED $COMMENT -- $UNCOMMENT SEQUENCE.'')')
     -      SOURCE(:IBLANK(SOURCE,80))
            call exit(3)
         end if
      else if (line(1:10).eq.'ENDCOMMENT') then
         if (comment) then
            comment = .false.
            if (nestc.ne.nestl) then
               write(*,'('' '',A//
     -         '' FATAL - UNMATCHED $IF...$ENDIF INSIDE $COMMENT...''
     -         ''$ENDCOMMENT BLOCK.'')')
     -         SOURCE(:IBLANK(SOURCE,80))
               call exit(3)
            end if
...
4     WRITE(*,'('' '',A//'' FATAL - INCOMPLETE STATEMENT.'')')
      ...
      PARAMETER (NUM=128)
      LOGICAL CONDOP(0:8), DC, WRITE, EB, comment
      CHARACTER DEFVAL(NUM)*31, DEFVAR(NUM)*31, SOURCE*80, LINE*80,
     1  VALUE*31
...
5     WRITE(*,'('' '',A,//,'' FATAL - CONSTANT LOGICAL EXPRESSION '',
     1  ''REQUIRED.'')') SOURCE(:IBLANK(SOURCE,80))
      ...
      LOGICAL CONDOP(0:8), ELSEFL(0:8), WRITE, EB, FALSE_ELSE, comment
      CHARACTER LINE*80, SOURCE*80
      COMMON /CONDOP/ CONDOP
      COMMON /ELSEFL/ ELSEFL
      COMMON /SOURCE/ SOURCE
      COMMON /WRITE/ NESTL, WRITE, comment, nestc
...
C ELSE DETECTED
        ELSEFL(NESTL)=.TRUE.
C VERIFY THAT WE'RE NOT IN AN ELSE BLOCK OF A TRUE IF
        FALSE_ELSE = .FALSE.
        DO 2 I = NESTL,1,-1
          IF (CONDOP(I).AND.ELSEFL(I)) THEN
            FALSE_ELSE = .TRUE.
          ENDIF
2       CONTINUE
        IF( .NOT. FALSE_ELSE) THEN
          CONDOP(NESTL)=.TRUE.
          WRITE=.TRUE.
        ENDIF
      ENDIF
...
1     WRITE(*,'('' '',A,//,'' FATAL - MISPLACED $ELSE OR $ELSEIF '',
     1  ''DIRECTIVE.'')') SOURCE(:IBLANK(SOURCE,80))
      ...
      LOGICAL CONDOP(0:8), ELSEFL(0:8), WRITE, EB, comment
      CHARACTER SOURCE*80
      COMMON /CONDOP/ CONDOP
      COMMON /SOURCE/ SOURCE
      COMMON /ELSEFL/ ELSEFL
...
C MIDDLE ITEM
      ...
      BLOCK DATA
      LOGICAL CONDOP(0:8), ELSEFL(0:8), WRITE, comment
      COMMON /CONDOP/ CONDOP
      COMMON /ELSEFL/ ELSEFL
      COMMON /NUMDEF/ NUMDEF
      COMMON /WRITE/ NESTL, WRITE, comment, nestc
      DATA WRITE /.TRUE./, CONDOP /.TRUE.,8*.FALSE./
     1 ELSEFL /9*.FALSE./, NUMDEF /0/, NESTL /0/
      END
